...
interface StateEvent {
  data: { enabled?: unknown; humanFallback?: unknown }
...
export interface AutoReviewRpcClient {
  ...
  setFallback(input: { humanFallback: boolean }, options?: unknown): Promise<unknown>
  events: {
...
export interface AutoReviewControllerOptions {
  ...
  /** Called only when a confirmed response or event changes the fallback. */
...
export interface AutoReviewController {
  ...
  enabled(): boolean
  /** Last human-fallback value confirmed by an RPC response or event. */
  ...
  /** Request a fallback switch. Returns whether the response confirmed a value. */
  setFallback(next: boolean): Promise<boolean>
  start(): void
  dispose(): void
}
...
export function createAutoReviewController(options: AutoReviewControllerOptions): AutoReviewController {
  ...
  let currentFallback = options.initialFallback ?? true
  let disposed = false
  ...
  const applyFallback = (value: unknown): boolean => {
    if (disposed || typeof value !== "boolean") return false
    if (value !== currentFallback) {
      currentFallback = value
      options.onFallbackChange?.(value)
    }
    return true
  }
  ...
  const fallbackOf = (response: unknown): unknown => {
    if (response === null || typeof response !== "object" || !("humanFallback" in response)) return undefined
    return (response as { humanFallback?: unknown }).humanFallback
  }
  ...
  const refresh = async (): Promise<boolean> => {
    ...
    applyFallback(fallbackOf(result))
  ...
  const setFallback = async (next: boolean): Promise<boolean> => {
    if (disposed) return false
    const sequence = ++requestSequence
    const revision = eventRevision
    let result: unknown
    let failed = false
    try {
      result = await options.client.setFallback({ humanFallback: next }, callOptions())
    } catch {
      failed = true
    }
    if (disposed) return false
    if (!failed && (sequence !== requestSequence || revision !== eventRevision)) return false
    if (failed || !applyFallback(fallbackOf(result))) {
      options.toast("Could not confirm the human fallback setting. Refreshing server state.")
      void refresh()
      return false
    }
    apply(enabledOf(result))
    return true
  }

  return {
    enabled: () => current,
    humanFallback: () => currentFallback,
    refresh,
    setEnabled,
    setFallback,
    start() {
      ...
      stopState = options.client.events.on("state", (event) => {
        ...
        eventRevision++
        applyFallback(event.data.humanFallback)
