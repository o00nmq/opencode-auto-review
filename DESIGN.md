...
| `text` | The model's next request, every later turn | Left empty, then removed from the dispatched request by the `context` hook, so the notice never enters the model's context |

OpenCode 2.0.4+ paints only `message.description` for a `type === "synthetic"` message, so notice text must live there to be visible at all. `text` is assembled into the following model request as a user message and replayed on every subsequent turn; it is not dropped by the host on its own. Verified on OpenCode 2.0.6 with an `http.request` probe in an isolated instance: a committed notice with `text: ""` arrives in the next request as a `user` message whose only part is empty text, and it accumulates across turns. Leaving that in place both leaks a review marker into the coding model's context and, in repeated runs, produced a degenerate zero-token request that returned an empty completion. Every automatic review notice therefore sends `text: ""` and carries its content in `description` only, and the plugin's own `context` hook filters that empty body out before dispatch (`isNoticeBody`). Matching on the empty-body shape, rather than a message id, also removes notices committed by an earlier plugin instance, whose ids a fresh instance cannot know. The notice stays in the session transcript and remains visible to the user; only the dispatch copy is dropped. The one exception is `/auto-review` control feedback (`showStatus`), which is a direct reply to a command the user typed and stays model-visible like any other command output.
